...
export const NOCCreationPanel: React.FC<NOCCreationPanelProps> = ({
  ...
  const [licenseNumber, setLicenseNumber] = useState(`ARGAAM-NOC-${new Date().getFullYear()}-${Math.floor(Math.random() * 10000)}`);
  ...
  const handleSelectApplication = (app: ApplicationFolder) => {
    ...
    setLicenseNumber(`ARGAAM-NOC-${new Date().getFullYear()}-${Math.floor(Math.random() * 10000)}`);
  };
  ...
  const handleGenerateNOC = async () => {
    ...
    if (nocAdditionalText && nocAdditionalText.trim()) {
      ...
      doc.setFont(fontFamily, 'normal');
      doc.setFontSize(11);
      const defaultBody = `This is to certify that Argaam has reviewed and approved the application from ${companyName} as per the submitted application and documentation.\n\nThis certificate is issued in accordance with the applicable regulations and company policies.`;
    ...
    doc.setFont(fontFamily, 'normal');
    doc.setFontSize(8);
    doc.text('Argaam', pageWidth / 2, y, { align: 'center' });
    y += 10;
    doc.text('Argaam, Riyadh, Saudi Arabia', pageWidth / 2, y, { align: 'center' });
    y += 10;
    doc.text('Tel: +966-11-1234567 | Email: info@argaam.com | Web: www.argaam.com', pageWidth / 2, y, { align: 'center' });
